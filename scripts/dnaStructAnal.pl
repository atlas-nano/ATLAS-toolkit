...
use General qw(FileTester Trim TrjSelections);
use AMBER qw(getTopInfo ParseAmberTrj getOpts GetAmberByteOffset);
use GetParms;
use HelixLayout;
use FileFormats qw(sortByRes);
use LAMMPS qw(ParseLAMMPSTrj GetLammpsByteOffset GetLammpsTrjType);
use Getopt::Std qw(getopt);
use ManipAtoms qw(UnwrapAtoms);
...
sub fixPDB {
    ...
    $cmd = "/ul/tpascal/scripts/stripNaH20.pl $pdbFile $pdbFile";
...
sub getHelicalParms {
	...
	chdir "helix${helixC}";
	$cmd = "/ul/tpascal/scripts/do_curve_anal.pl ../pdbfiles/" . $filebase . 
	    " 1 " . ($HC->{$i}{COUNT} - 1) . " $helixC " . ($HC->{$i}{LEN}/2);
	die "ERROR: Command aborted: $cmd" if (system("$cmd >& junk"));
	
	$cmd = "/ul/tpascal/scripts/get_helical_parms_new.pl " . $filebase .
