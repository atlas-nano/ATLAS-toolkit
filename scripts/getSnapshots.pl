...
use AMBER qw(ParseAmberTrj getTopInfo getOpts getConn GetAmberByteOffset);
use General qw(FileTester IsInteger GetSoluteAtoms CoM CenterOnMol);
use FileFormats qw(createBGF createHeaders addHeader GetBondList);
use GetParms;
use ManipAtoms qw(ImageAtoms);
