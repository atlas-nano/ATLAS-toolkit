...
my ($iter,$best_correl,$angle,$ni, $best_angle);
...
for $curr_file (@FILES) {
	...
	for $iter (1 .. $ni) {
		$angle = 180 - 360/$iter;
		$grace_batch = CreateBatchFile($curr_file, $valence, $angle);
		next if ($grace_batch eq "");
		...
		next if (! $PARMS->{"WASSUCESSFUL"});
		if (exists($PARMS->{STATS}{Correlation}) and $PARMS->{STATS}{Correlation} > $best_correl) {
			$best_correl = $PARMS->{STATS}{Correlation};
			$best_angle = $angle;
		}
		print "...";
	}
	$grace_batch = CreateBatchFile($curr_file, $valence, $best_angle);
	if ($grace_batch eq "") {
...
sub CreateBatchFile(@) {
	...
	if ($valence_term == 1) {
		...
		$FORMULAR	= "y = ($sqrtPI*$rt2)/(2*a1)*exp(-a1*(x-a2)^2/$rt2)";
	} elsif ($valence_term == 2) {
		$TITLE	   = "ANGLES";
		$AXISLABEL   = "BENDING ANGLE (degrees)";
		#$FORMULAR	= "y = a0*exp(-a1*(cos(x deg) - cos(a2 deg))^2/$rt2)";
		$FORMULAR	= "y = ($rt2/a1)*erf(3.141592)*exp(-a1*(x - a2 deg)^2/$rt2)";
		#$FORMULAR	= "y = a0*exp(-a1*(cos(x deg) - cos(a2 deg))^2/$rt2)";
		#$FORMULAR	= "y = a0*exp(-a1*(1 - cos(x - a2 deg))/$rt2)";
	} else {
		$TITLE	   = "TORSIONS";
		$AXISLABEL   = "TORSION ANGLE (degrees)";
		$FORMULAR	= "y = a0*exp(-a1*(1 + cos(a2*x - a3 deg))/$rt2)";
		$FORMULAR	= "y = ($rt2/a1)*erf(3.141592)*exp(-a1*(1 + cos(a2*x - a3 deg))/$rt2)";
		#$FORMULAR	= "y=a0*exp(-( a1*(1-cos(1*x deg)) +  a2*(1-cos(2*x deg)) +  a3*(1-cos(3*x deg)) +  a4*(1-cos(4*x deg)) +  a5*(1-cos(5*x deg)) +  a6*(1-cos(6*x deg)))/(2*0.0019872*298))";
	...
	$out_string = "";
	($MESHDATA, $min, $max, $counter, $nbin) = GetMesh($cfile, $valence_term);
	return "" if ($counter < 20);
	if ($valence_term == 1) {
		$constraint .= "a2 = ". ($min+($max-$min)/2) . "\na2 constraints off\n";
		$constraint .= "a1 = 500\n" if ($cfile =~ /(H|D)\d/);
	} elsif($valence_term == 2) {
		$constraint = "a2 = IMAX(s1.y)*" . ($max - $min)/$nbin . "\n";
...
sub CreatePics(@) {
	...
	for $counter (values %{ $pic_list}) {
		...
		$pic_file =~ s/\.\w+$//g;
		$pic_file .= "_" . $index . ".png";
		$pic_file = "pics/" . $pic_file;
		$exec_string = "$grace_cmd $counter -maxpath 1000000 -hardcopy -hdevice 'PNG' -printfile '" . $pic_file . "'";
		system($exec_string);
		$index++;
...
sub PrintData(@) {
	my ($data) = $_[0];
	my ($valence, $file_nm, $parm, $out_text, $tmp, $tmp1, $val);
	...
	for $valence (keys %{ $data }) {
		...
		for $file_nm (sort sortString keys %{ $data->{$valence} }) {
			...
			for $parm ("a0", "a1", "a2", "a3", "a4", "a5", "a6") {
				if (defined($data->{$valence}{$file_nm}{$parm})) {
					$val = $data->{$valence}{$file_nm}{$parm};
					if ($valence > 1 and $parm eq "a3") {
						$val -= 360 while($val>306);
						$val += 360 while($val<0);
					}
					printf "%15.4f", $val;
				}
...
sub GetMesh(@) {
	...
	return ($returnstr, $min_val, $max_val, $counter, $num_bins);
