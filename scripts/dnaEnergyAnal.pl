...
use General qw(FileTester Trim execCmd PrintProgress STDev TrjSelections GetSoluteAtoms CoM CenterOnMol GetSelections);
use GetParms;
use HelixLayout;
use FileFormats qw(GetBGFFileInfo createBGF addHeader addBoxToHeader createHeaders);
...
use ManipAtoms qw(UnwrapAtoms ImageAtoms GetAtmList GetAtmData SplitAtomsByMol);
use Cwd 'abs_path';
use Cwd;
...
my ($molList, $SOLUTEATMS, $getByteOffset, $LAMMPSOPTS, $molName);
...
for (keys %{ $BGF }) {
    $molList->{$_} = 1 if (! exists($SOLUTEATMS->{$_}));
}
...
$Parm->{Molecule}{name} = $molName if (defined($molName));
...
sub saveBGF {
	...
	$BBOX = ConvertLammpsBox($ATOMS->{"BOX BOUNDS"});
	$ATOMS = $ATOMS->{ATOMS};
	...
	$CENTER = CoM($MOL);
        ...
        for $j (@tmp) {
            $BBOX->{$j}{CENTER} = $BBOX->{$j}{len}/2;
            $BBOX->{$j}{hi} = $BBOX->{$j}{len};
            $BBOX->{$j}{lo} = 0;
            for $i (keys %{ $ATOMS }) {
                $ATOMS->{$i}{$j} += ($BBOX->{$j}{CENTER} - $CENTER->{$j});
            }
        }
        for $i (keys %{ $molList }) {
            $MOL = GetAtmData($ATOMS, $molList->{$i});
            $CENTER = CoM($MOL);
            ImageAtoms($MOL, $CENTER, $BBOX);
        }
    }
    $headers = createHeaders($BBOX, $bgfFile);
...
sub init {
    ...
    getopt('bpstacin',\%OPTS);
    ($parmFile, $bgfFile, $filebase, $selection, $atomSelection, $trjFile, $reImageCoords, $molName) = 
	($OPTS{p},$OPTS{b},$OPTS{s},$OPTS{t},$OPTS{a},$OPTS{c}, $OPTS{i}, $OPTS{n});
	...
	$RES->{RESNUM}{"<" . ( $Parm->{Molecule}{total_bases} + 1) . ""} = 1;
    #}
