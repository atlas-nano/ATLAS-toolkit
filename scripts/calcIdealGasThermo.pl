...
sub calcIdealGasThermo {
	...
	$data->{THERMO}{ENTROPY}{TRANS} = $R*(log((2*$PI*$mass*$k*$temp/$h/$h)**(3/2)*($k*$temp)/$press)+5/2);
...
sub minimizeStructure {
	...
	my ($execCmd, $prefix, $tot, $i, $tmp, $I_prod, $Internal_E, $c_len, $EMD, $shakeCmd);
...
$shakeCmd
run 0
velocity all create ${cell_temp} 9838238 dist gaussian
thermo_style multi

fix             3 all nve
fix             2 all langevin ${cell_temp} ${cell_temp} 10.0 51863342
#run             20000
unfix           2
unfix           3
#fix             2 all gle 6 ${cell_temp} ${cell_temp} 12345 matrixA
#run             80000
#unfix           2
...
sub init {
	...
	getopt('bfpvtrms',\%OPTS);
	for ("b", "f") {
	die "usage: $0 -b bgf file -f force field -v (cell_volume - cubic angstroms to calculate pressure) -p (cell_pressure - bar) -t (cell_temperature - Kelvin) -r (rot_sym_number = 2) -m (vibrational_modes_file) -s (use_shake=no|0)\n"
		if (! exists($OPTS{$_}));
	}

	print "Initializing...";
	($bgfFile, $ffList, $cell_vol, $cell_press, $cell_temp, $rsigma, $modes_file, $doShake) = 
		($OPTS{b}, $OPTS{f}, $OPTS{v}, $OPTS{p}, $OPTS{t}, $OPTS{r}, $OPTS{m}, $OPTS{s});
	FileTester($bgfFile);
	...
	$lmp_cmd = "/home/tpascal/codes/bin/lmp_serial"; #change here
}
