...
use General qw(FileTester Trim GetBondLength GetAngle GetTorsion CenterText STDev);
use File::Basename qw(basename dirname);
use MESO qw(GetMesoParms CreateMesoModel MakeMesoBGF);
use Math::Matrix;
use BOX qw(CreateGrid GetNeighbours GetBox);
use Cwd qw(getcwd);
...
sub main {
	...
	my ($CONS, %POS_VECS, $BEADS, $PARMS, $rec, $BOX);
	...
	for $i (@{ $BGF_FILES }) {
		...
		&updateStats($rec, $PARMS, \%BONDS, \%ANGLES, \%TORSIONS, \%HBONDS, $BOX);
		print "Finished\n";
		$fC++;
		$rec = ();
		$ATOMS = ();
...
sub updateStats {
	my ($cFile, $parms, $BONDS, $ANGLES, $TORSIONS, $HBONDS, $box) = @_;
	my ($a1, $a2, $a3, $a4, $i, $j, $k);
	my ($l, $sKey, @atmList, $sName, $BOX);
	...
	for $i (keys %{ $cFile->{"ATOMS"} }) {
		...
		for $j (@{ $cFile->{"BONDS"}{$i} }) { #BONDS
			...
			for $k (@{ $cFile->{"BONDS"}{$j} }) { #ANGLES
				...
				@atmList = ($a1, $a2, $a3);
				($sKey, $sName) = getKeyList(\@atmList, $parms);
				$ANGLES->{$sKey}{"VALS"} .= GetAngle($a1, $a2, $a3, $box, 0) .  " ";
				...
				for $l (@{ $cFile->{"BONDS"}{$k} }) { #TORSIONS
					...
					@atmList = ($a1, $a2, $a3, $a4);
					($sKey, $sName) = getKeyList(\@atmList, $parms);
					$TORSIONS->{$sKey}{"VALS"} .= GetTorsion($a1, $a2, $a3, $a4, $box, 0) . " ";
				...
				for $hb (@hList) {
					...
					for $cell (@{ $CELLS }) {
						for $atom (@{ $cell->{"ATOMS"} }) {
							...
							if ($atom->{"FFTYPE"} =~ /H_\d+/) { #Acceptor
								$sType = $atom->{FFTYPE};
								($hb_valid, $hb_dist, $hb_angle) = validHB($hb->{"PARENT"}, $hb->{"ATOM"}, $atom, $BOX);
								if ($hb_valid) {
									$saveName = "$hb->{ATOM}{FFTYPE}-$hb->{PARENT}{ATMNAME}-${sType}_dist";
									$saveKey = sprintf("%03d%03d%03d", $hb->{ATOM}{ID}, $hb->{PARENT}{ID},$atom->{ID}); 
									$HBOND->{$saveKey}{"VALS"} .= "$hb_dist ";
									$HBOND->{$saveKey}{"NAME"} = $saveName;
									$saveName = "$hb->{ATOM}{FFTYPE}-$hb->{PARENT}{ATMNAME}-${sType}_angl";
									$saveKey += 1000000000;
...
sub validHB {
	my ($parent, $donor, $acceptor, $box) = @_;
	my ($valid, $dist, $angle);	
	
	$valid = 0;
	
	$dist = GetBondLength($parent, $acceptor, $box);
	$angle = GetAngle($parent, $donor, $acceptor, $box, 0);
	
	if ($dist < 6 and $angle > 120 and $angle < 240) {
		$valid = 1;
	}
