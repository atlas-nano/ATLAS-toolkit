...
use General qw(FileTester Trim LoadElements AddElementField LoadFFs ReadFFs);
use FileFormats qw(GetBGFFileInfo createMSI);
use BOX qw(GetBox);
...
my ($bgf_file, $ffList, $save_name) = @ARGV;
my ($ATOMS, $BBOX, $MSI, $BONDS, $HEADERS, $PARMS, $ELEMENTS, $FFILES);

$|++;
print "Initializing...";
&init;

#print "Parsing forcefield ${FFILES}...";
$PARMS = LoadFFs($FFILES);
print "Parsing BGF file $bgf_file...";
($ATOMS, $BONDS, $HEADERS) = GetBGFFileInfo($bgf_file, 1);
&AddElementField($ATOMS, $ELEMENTS, $PARMS);
