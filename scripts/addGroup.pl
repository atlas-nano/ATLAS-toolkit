...
use General qw(GetSelections FileTester CombineMols PrintProgress);
...
sub addGroupToStructure {
    ...
    my ($i, @tmp, $atmCounter, $OFFSET, $currGroup, $j, $start, $printStr, @junk); 
    my ($alternate, $k, @atmStack, $currAtom, $rec, $counter, $strLen, $totAtms);

    @tmp = sort numerically keys %{ $atomSelection };
    $totAtms = scalar @tmp;
    $atmStack[0]{"ATOM"} = shift @tmp;
    $atmCounter = pop(@tmp) + 1;
    @tmp = sort numerically keys %{ $atoms->{GROUP} };
    $counter = 0;
    $start = time();
    $printStr = "Adding Groups to structure...";

    print "{$printStr}Calculating time remaining\r";
    while (@atmStack) {
	$currAtom = pop @atmStack;
	$i = $currAtom->{ATOM};
	next if (exists($atoms->{STRUCTURE}{$i}{ALTPOS}));
	delete $atomSelection->{$i};
	if (exists($currAtom->{PARENT}) and exists($atoms->{STRUCTURE}{ $currAtom->{PARENT} }{ALTPOS})) {
	    $alternate = -1 * $atoms->{STRUCTURE}{ $currAtom->{PARENT} }{ALTPOS};
	} else {
	    $alternate = 1;
	}
	...
	for $j (@{ $bonds->{STRUCTURE}{$i} }) {
	    next if (! exists($atomSelection->{$j}));
	    $rec = (
			{
			    "ATOM"   => $j,
			    "PARENT" => $i,
	    ...
	    unshift @atmStack, $rec;
	}
	$counter++;
	$strLen = PrintProgress($counter, $totAtms, $start, $printStr);
	if (! @atmStack and keys %{ $atomSelection }) {
	    @junk = sort numerically keys %{ $atomSelection };
	    $atmStack[0]{"ATOM"} = shift @junk;
	}
    }
...
sub showUsage {
    return "usage: $0 -s structure file -g group file  -a atom selection\n" . 
