...
sub ReadJagOut(@) {
    ...
    my (%Jaguar_Data, $resCounter, $is_radii);

    $is_radii = $is_geometry = $is_charge =  $is_valid = $is_radii = 0;
    $resCounter = 1;
...
#			print "Wrote Charge\n";
	    ...
	    $is_radii = 0;
	} elsif ($is_radii and $inline =~ /^\s*(\w+)\s+(\d+\.\d+)/) {
	    $counter++;
	    $Jaguar_Data{$resCounter}{$counter}{"RADII"} = $2;
	}
    }
    close JAGOUT;
