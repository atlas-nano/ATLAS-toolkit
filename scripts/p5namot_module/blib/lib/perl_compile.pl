...
$ENV{CPPFLAGS} = "" if (!exists($ENV{CPPFLAGS}));
$ENV{LDFLAGS} = "" if (!exists($ENV{LDFLAGS}));
$ENV{CFLAGS} = "" if (!exists($ENV{CFLAGS}));
$ENV{CFLAGS} .= " -Wformat=0 -Wno-format-security -Wno-int-to-pointer-cast";
for $count (0 .. $#mylist) {
    $compilename =  substr(basename($mylist[$count]),0,-2) . ".o";
    $outstring = 'gcc -DHAVE_CONFIG_H -I. -I. -I..  -fpic -Wformat-security ' .
                 $ENV{CPPFLAGS} . " -I${Install_loc} -I${Install_loc}/../" .
				 ' -DLIB_HOME="\"/home/tpascal/codes/namot2/v2.2.0-pre4/share/namot\"" -DHELP_FILE_DIR="\"/home/tpascal/codes/namot2/v2.2.0-pre4/share/namot\"" -I/usr/include/python2.7 -I/usr/lib/python2.7/config';
...
print MAKEFLE "use ExtUtils::MakeMaker;\n" .
              ...
              "             'NAME'    => 'p5namot'," .
              "                 \# Name of package\n" .
			  "             'INC' => '-I${Install_loc} -I${Install_loc}/../ $ENV{CPPFLAGS} $ENV{CFLAGS}',  \n" .
              "             'LIBS'    => ['$ENV{LDFLAGS}" . 
			  "-lutil -ldl -lpthread -lpng -lm -lXm -lXt -lXext -lgsl -lgslcblas -lX11  -lSM -lICE'],  " .
