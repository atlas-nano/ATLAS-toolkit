...
use GROMACS qw(ParseGroFF UpdateGroFFData);

sub parseGromacsFF;
sub updateGroParams;
sub addFields;

my ($ffFile, $itpFile, $saveName);
my ($PARMS, $FF, $ELEMENTS, $itpType);

$|++;
&init;
print "Parsing GROMACS ITP file $itpFile...";
$PARMS=ParseGroFF($itpFile, $itpType);
&createTypeData($PARMS);
if(defined($ffFile)) {
	print "Done\nParsing supplementary file $ffFile...";
	&updateGroParams($PARMS, $ffFile);
}
print "Done\nCreating CERIUS2 formatted forcefield $saveName...";
if ($itpType == 1) {
	$FF = UpdateGroFFData($PARMS, $ELEMENTS, undef);
} else {
	$FF = $PARMS;
}
...
sub init {
	...
	print "Initializing...";
	getopt('fist',\%OPTS);
	($ffFile,$itpFile,$saveName,$itpType) = ($OPTS{f}, $OPTS{i}, $OPTS{s}, $OPTS{t});
	die "usage: $0 -i gromacs_itp_file -f (gromacs_force_field) -s (savename)\n"
		if (! exists($OPTS{i}));
	FileTester($ffFile) if (defined($ffFile));
	FileTester($itpFile) if (defined($itpFile));
	$itpType = 1 if (! defined($itpType) or $itpType =~ /1/);
	$itpType = 2 if ($itpType =~ /2/);
	if (! defined($saveName)) {
