...
my ($bgfFile, $msiFile, $FF, $suffix, %OPTS, $inputType, $ScaleTorsionList, $reaxFF, $opts, $pQeQopt);
my ($ATOMS, $CONS, $HEADERS, $FILES, $PARMS, %ERRORS, $i, $BOX, $ffType, $ismesodna);
...
&printAtoms($ATOMS, $BOX, \*DATFILE, $pQeQopt);
...
&writeQEqParms($PARMS, $suffix) if (exists($PARMS->{QEq}) and keys %{ $PARMS->{QEq} });
...
sub writeQEqParms {
	...
	for $index (1 .. $count) {
		...
		printf QEqFile "%-5d %8.4f %8.4f %8.4f %8.4f %8.4f\n",$index,$curr->{CHI}*2,$curr->{ETA},$curr->{GAMMA},$curr->{ZETA},$curr->{CHRG};
	}
	close QEqFile;
}
...
sub printAtoms {
	...
	$fmt = "%8d %8d %10.5f %10.5f %10.5f %8d %11.5f %11.5f %5d %5d %5d %4d %4d %4d\n" 
		if($pqeq); #hybrid full pqeq: atom-ID atom-type x y z molecule-ID q q Rsx Rsy Rsz
	for $dim ("X", "Y", "Z") {
...
sub determineIfHybrid {
	...
	for $i (keys %vdwType) {
		...
		for $j (@tmp) {
			...
			next if (! $jType);
			for $k (@tmp) {
				next if (exists($parms->{$j}{$k}) and keys %{ $parms->{$j}{$k} });
...
sub init {
	...
	$totAtms = 0;
	$pQeQopt = 0 if (!defined($pQeQopt) or $pQeQopt !~ /1|yes/i);
	$pQeQopt = 1 if($pQeQopt =~ /1|yes/i);
	print "Done\n";
...
usage: $0 -b bgfFile -f \"ff1 ff2...\" -s [suffix] -o [options] -t [sim type] -r [reaxff] -q [pqeq=no]
	...
	-q [pqeq]: (optional) create PQeQ compatible lammps files. Default is no
