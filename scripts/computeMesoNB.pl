...
use AMBER qw(ParseAmberTrj getOpts getTopInfo GetAmberByteOffset);
use General qw(FileTester TrjSelections CoM GetSelections GetBondLength STDev);
...
sub showUsage {
	...
	"\t-e eng file: location of the energy per timestep file (created with /ul/tpascal/scripts/dnaEnergyAnal.pl)\n" .
