...
# now determine the number of atoms before the first turn
    ...
    -e "/ul/tpascal/scripts/function.pl" or die "Cannot locate file function.pl, so cannot calculate twist angles\n";

    open TWISTANGLE, "/ul/tpascal/scripts/function.pl $major_no $minor_no |" or die "Cannot execute function.pl";
    while (<TWISTANGLE>) {
	chomp;
	push @temptwist_angle_data, $_;
    }

    for $i ($major_no .. ($minor_no + $major_no -1)) {
        push @twist_angle_data,  $temptwist_angle_data[$i];
    }
