...
@ISA = qw(Exporter);
@EXPORT = qw(parseAmberLib getTopInfo updateAtmLabels parseCoordFile getConn CreateAmberTrj
	     getOpts loadCnvFile parseAmberFF centerAtoms GetVels ParseAmberTrj AmberLib GetAmberByteOffset);
$VERSION = "1.00";

sub parseAmberLib {
    my ($parmFile) = $_[0];    
    my ($parmName, $parmType, @HEADERS, %PARMS, $parmCounter, @VALS, $i);
	...
	if ($_ =~ /^\!entry\.(\w+)\.\w+\.(\w+)\s+(\w+)\s+(.+)/) {
        ...
        } elsif ($parmType =~ /Orders/ and $_ =~ /^\s\"(\S+)\"/ and $parmCounter) {
            $PARMS{$parmType}{$parmCounter} = $1;
            $parmCounter++;
	} elsif ($_ =~ /^\s\"(\S+)\"/ and $parmCounter) {
            @ATOMS = @VALS = ();
            $tmp = $1;
            if ($tmp eq "?") {
                $tmp = "X";
...
sub addResInfo {
	...
	if ($startAtm == 0) {
		...
		$ATOMS->{$atomC}{"CHARGE"} = $ATOMS->{$atomC}{"CHARGE"}/18.2223;
...
sub getOpts {
    my ($inFile) = "/net/hulk/home3/tpascal/scripts/dat/amberTopFile.perldata";
    my (%OPTS);
...
sub parseAmberFF {
    ...
    my ($rec, $torsionCounter, $inversionCounter, $section, $i, @tmp, $atom1, $pi);
