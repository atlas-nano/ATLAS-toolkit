...
sub loadConverter {
    my ($inFile) = "/net/hulk/home3/tpascal/scripts/dat/ceriusParms2Lammps.perldata";
    my (%SHASH);
...
sub parseCerius2FF {
    ...
    $PARMS{PARMS}{tip4_om_dist} = 0.125;
    $type_counter = 0;
...
#			$vdws[1] = $vdws[1]/2;
		...
		$currParm->{$vdw_counter} = (
						 {
						     "TYPE"   => $vdwType,
						     "Lammps" => getLammpsOpts($vdwType,"vdw", $CNV, $istip4p),
						     "KEY"    => "$atom1 $atom2 ",
						     "VALS"   => [@vdws],
						     "ATOM"   => "$atom1 $atom2 ",
						     "USED"   => 0,
...
sub ReadFFs {
    ...
    for $i (@tmp) {
	    ...
	    $currFF = "AMBER";
            ...
            $rec = (
                        {
                            "FF"      => "/ul/tpascal/ff/AMBER_MBSC0.ff",
                            "FFTYPE"  => "CERIUS2",
                        }
                    );
            $ffType = 1 if (! defined($currFF) or $currFF eq "AMBER");
            ...
            $rec = (
                        {
                            "FF"      => "/ul/tpascal/ff/charmm_par_all27_lipid.ff",
                            "FFTYPE"  => "CERIUS2",
                        }
                    );
            ...
            $rec = (
                        {
                            "FF"      => "/ul/tpascal/MesoDNA/ff/MesoDNA.ff",
                            "FFTYPE"  => "CERIUS2",
                        }
                    );
            $ffType = 3;
