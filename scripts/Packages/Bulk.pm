...
use constant a2m => 1E-10; # 1 A = 10^-10 m
...
use constant qa22esu => 1.345E-26; # q x A^2 -> e.s.u
use constant qa22Cm2 => 4.486554E-40; # q x A^2 -> C x m^2
use constant e0 => 8.854187817E-12; # vacuum permitivity A^2 s^4 kg^-1 m^-3
use General qw(CrossProduct CenterText GetEquilPoint GetStats);
use Math::MatrixReal;
...
@EXPORT_OK = qw(CalcMolOrientCorrelation CalcDipole CalcDielectricConstant CalcDiffusionConstant SetOpts
		GetOrientOpt CalcQuadrupole CalcHVap CalcHCap CalcTCompress CalcTExpand GetFluct WriteStats WriteData);
$VERSION = "1.00";
...
sub CalcDipole {
    ...
    for $i (keys %MOL_DIPOLE) {
    	for ("X", "Y", "Z") {
	        $dMoment->{$_} += $MOL_DIPOLE{$i}{$_};
    	}
    }
    for ("X", "Y", "Z") {
	    $dMoment->{$_} /= (scalar keys %MOL_DIPOLE);
    }
    $dMoment->{T} = sqrt(($dMoment->{X}**2) + ($dMoment->{Y}**2) + ($dMoment->{Z}**2));
    $DIPOLE{u} = $dMoment->{T};
    $DIPOLE{u2} = $dMoment->{T} * $dMoment->{T};
...
sub CalcQuadrupole {
    my ($atoms, $mols, $data, $doMoments) = @_;
	my (@tmp, $factor, $k, $l, $del, %QUADRUPOLE);
	my ($i, $j, $r2, $qp, $matrix, $eigen, $ret, $avgqP, $ansioqP);

    @tmp = ("X", "Y", "Z");

	#$factor = qa22esu *1E26; #units of 10^26 esu
	$factor = qa2Cm * debye;
    for $k (@tmp) {
		for $l (@tmp) {
			$del = 1;
			...
			for $j (keys %{ $mols }) {
				...
				for $i (keys %{ $mols->{$j}{MEMBERS} }) {
					$r2=$atoms->{$i}{XCOORD}**2+$atoms->{$i}{YCOORD}**2+$atoms->{$i}{ZCOORD}**2;
					#$qp = $factor * $atoms->{$i}{CHARGE} * (3*$atoms->{$i}{"${k}COORD"}*$atoms->{$i}{"${l}COORD"}-$r2*$del);
					$qp = $factor * $atoms->{$i}{CHARGE} * $atoms->{$i}{"${k}COORD"}*$atoms->{$i}{"${l}COORD"};
					$QUADRUPOLE{$k}{$l} += $qp;
				}
			}
		}
	...
	$matrix = Math::MatrixReal->new_from_rows([
												[$QUADRUPOLE{X}{X},$QUADRUPOLE{X}{Y},$QUADRUPOLE{X}{Z}],
												[$QUADRUPOLE{Y}{X},$QUADRUPOLE{Y}{Y},$QUADRUPOLE{Y}{Z}],
												[$QUADRUPOLE{Z}{X},$QUADRUPOLE{Z}{Y},$QUADRUPOLE{Z}{Z}],
											  ]);
	$eigen = $matrix->sym_eigenvalues();
	$avgqP = $eigen->[0][2][0]/2;
	$ansioqP = $eigen->[0][0][0]-$eigen->[0][2][0];
	$ret = (
			{
				"AVG"   => $avgqP,
				"ANISO" => $ansioqP,
	...
	for $i (1 .. $#tmp) {
		for $j ($i .. $#tmp) {
			$ret->{$tmp[$i] . $tmp[$j]} = $QUADRUPOLE{$tmp[$i]}{$tmp[$j]};
		}
	}
	return $ret;
}
