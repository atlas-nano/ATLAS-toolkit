...
sub PlaceAtomsOnGrid {
	my ($ATOMS, $GRID, $BOX, $grid_len, $is_solv) = @_;
	my ($counter, $AtomC, $Index, $dim, @deleted_keys, $i, $radii);  
	my ($atom, $cell, $maxVol, $exclude, $atomVol, $burried);
	my ($pi) = atan2(1,1) *4;
	
	$maxVol = $grid_len**3;
	$radii = 3; #default atom radii
	$is_solv = 0 if (! defined($is_solv));
	...
	while ($#deleted_keys > -1) {
		...
		if ((exists($atom->{IS_SOLVENT}) and $atom->{IS_SOLVENT}) or $is_solv) {
			...
			$radii = $atom->{"RADII"} if (defined($atom->{"RADII"}));
			$atomVol  = (4 * $radii**3 * $pi)/3;
...
sub GetNeighbours {
	my ($GRID, $CELL, $layers, $FROZEN, $dL) = @_;
	my (@CLIST, $i, %MAPING, $j, $count, $CONSTANT, $d);
	my ($xI, $yI, $zI, $k, $tot, $g, $VALS, $rec, @DLIST);
	...
	for $i (@{ $MAPING{X} }) {
		for $j (@{ $MAPING{Y} }) {
			next if (! exists($GRID->{$i->{g}}{$j->{g}}));
			for $k (@{ $MAPING{Z} }) {
				if(exists($GRID->{$i->{g}}{$j->{g}}{$k->{g}})) {
					$g = $GRID->{$i->{g}}{$j->{g}}{$k->{g}};
					if(exists($g->{ATOMS}) or exists($g->{IONS}) or exists($g->{WATERS})) {
						push @CLIST, $GRID->{$i->{g}}{$j->{g}}{$k->{g}};
						$rec = ([$i->{d},$j->{d},$k->{d}]);
						push @DLIST, $rec;
					}
				}
...
sub GetNeighbours_old {
	...
	for $i (@{ $MAPING{X} }) {
		next if (!exists($GRID->{$i}));
		for $j (@{ $MAPING{Y} }) {
			next if (! exists($GRID->{$i}{$j}));
			for $k (@{ $MAPING{Z} }) {
				if(exists($GRID->{$i}{$j}{$k})) {
					$g = $GRID->{$i}{$j}{$k};
					push @CLIST, $GRID->{$i}{$j}{$k} if(exists($g->{ATOMS}) or exists($g->{IONS}) or exists($g->{WATERS}));
				}
			}
		}
	}
...
sub GetBox {
	my ($ATOMS, $PARMS, $HEADERS, $padding) = @_;
	...
	if (! $isValid) {
		for $i (keys %{ $ATOMS }) {
			undef $radii;
			$radii = GetRadii($ATOMS->{$i}, $PARMS) if (defined($PARMS));
			if (! defined($radii)) {
				$radii = 3; #default
			}
			$radii += $padding if (defined($padding));
...
sub GetRadii {
	my ($atom, $PAR) = @_;
	my ($returnVal, $atmName, $tmp);

	$atmName = $atom->{"FFTYPE"};
	if (! $PAR or ! exists($PAR->{"VDW"}{$atmName}) or ! exists($PAR->{"VDW"}{$atmName}{$atmName})) {
		$atmName = $atom->{"ATMNAME"};
	} else {
		@{ $tmp } = keys %{ $PAR->{"VDW"}{$atmName}{$atmName} };
		$returnVal = $PAR->{"VDW"}{$atmName}{$atmName}{$tmp->[0]}{"VALS"}[1]/2;
	}
...
sub GetNlist {
	...
	for $i (keys %{ $select }) {
		...
		for $j (keys %{ $select }) {
			...
			$atom2 = $atoms->{$j};
			$rec = ();
			&General::GetMinDist($atom1, $atom2, $box, \%{ $rec });
			$dist = 0.0;
			for $k ("X","Y","Z") {
				$dist += $rec->{$k}*$rec->{$k};
			}
