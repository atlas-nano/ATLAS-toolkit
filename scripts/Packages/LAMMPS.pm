...
sub CreateInputFile {
	...
	my ($qeqDump, $electrodeQStr, $electrodeStr, $cmapStr);
...
DATA
	...
	if(exists($parms->{PARMS}{RIGID}) and keys %{ $parms->{PARMS}{RIGID}{MOLS}}) {
		...
		undef $tmp;
		$tmp = 0;
		map { $tmp++ if($parms->{PARMS}{MOLS}{$_}{MOLSIZE} > 9) } keys %{ $parms->{PARMS}{MOLS} }; 
		...
		if(exists($parms->{PARMS}{RIGID}{MOLS}{all})) {
			...
			if($polarizable and $parms->{PARMS}{POLARIZATION}{type}>0) { 
				if($parms->{PARMS}{POLARIZATION}{rigidOpt} ne "ATOMS") {
					$middle .= "\ngroup rtmp molecule " . getMolRange($tmp) . 
								"\ngroup RIGID subtract rtmp SHELLS" . 
								"\ngroup FLEX subtract ATOMS RIGID\n";
				} else {
					$middle .= "\ngroup RIGID molecule " . getMolRange($tmp) . "\ngroup FLEX subtract ATOMS RIGID\n";
				}
			} else {
...
sub readTail {
	...
	my ($tmp1, $tmp2, $unfixShellid, $sstr);
	...
	foreach $inputFile (@{ $parms->{PARMS}{INPUTLOC} }) {
		...
		while (<TAIL>) {
			...
			if($rigidMols ne "none") {
				$inStr = "" if ($inStr =~ /shakeOpts/ and $flexMols == "");
				$inStr = "" if ($inStr =~ /minimize/);
				if($inStr =~ /^\s*fix\s+(\S+)\s+(\S+)\s+(n.t|nph|nve)\s*(.*)$/i) {
					...
					$inStr = "";
				} elsif ($inStr =~ /^unfix\s+(\S+)/ and $flexMols ne "") {
					$inStr .= "\nunfix ${1}_flex";
				}
			}
			if ($inStr =~ /shakeOpts/) {
			...
			if($polarizable) {
				...
				if ($parms->{PARMS}{POLARIZATION}{type}>0) {
					...
					if($inStr =~ /fix\s+(\S+)\s+($sstr)\s+(rigid.npt|rigid.nvt|npt|nvt)/) {
						...
						$inStr .= "\nfix ${tmp1}_shells SHELLS nvt temp \${drude_T} \${drude_T} 20";
						$inStr .= "\nfix INVERSE all drude/transform/inverse";
						$unfixShellid = $tmp1;
					} elsif($inStr =~ /fix\s+(\S+)\s+($sstr)\s+(rigid|nve)/) {
						$tmp1 = $1;
						$inStr =~ s/all/ATOMS/;
						$inStr .= "\nfix ${tmp1}_shells SHELLS nve";
						$unfixShellid = $tmp1;
...
sub createInputHeader {
	...
	my ($header, $i, $parm, $j, $tmp, $hasCharmmTorsions);
