...
use FindBin qw($Bin);
use lib "$FindBin::Bin";
use General qw(PrintProgress LoadConverter FindElementByMass);
...
@ISA = qw(Exporter);
@EXPORT = ();
@EXPORT_OK = qw(parseAmberLib getTopInfo updateAtmLabels parseAmberCoordFile getConn CreateAmberTrj ConvertAmberBox
...
sub getOpts {
	my ($inFile) = "$Bin/dat/amberTopFile.perldata";
	my (%OPTS);
...
sub parseAmberFF {
	...
	my ($CNV, $elements, $in_data, $which_var, $type_counter, $EQUIV, $tmp);
	my (%PARMS, $eleNum, $rec, $i, @aTypes, @vals, $validFF, $curr, $validEquiv);
	...
	while (<FORCEFIELD>) {
		chomp;
		$in_data = $_;
		if ($in_data =~ /^(MOD4|NONBON|%FLAG)/) {
			$which_var = 5; #vdw
		} elsif ($which_var == 5 and $_ =~ /^END$/) {
			$which_var = -1;
			last;
		} elsif ($which_var =~ /^\s*$/) {
			$which_var = -1;
		} elsif($which_var != 5 and $in_data =~ /^\s*([^\.]+)\s+(\d+\.\d*)/) {
			$tmp = $1;
			@aTypes = split /\-/, $tmp;
			@aTypes = ($tmp) if ($tmp =~ /\-\s*$/); #fix for atoms with xx- in name (mostly ions)
			for $i (0 .. $#aTypes) {
				$aTypes[$i] =~ s/^\s*//;
				$aTypes[$i] =~ s/\s*$//;
			}
			$which_var = scalar(@aTypes);
			$type_counter = 0;
		} elsif ($_ =~ /^MASS$/) {
			$which_var = 1;
		} elsif ($_ =~ /^BOND$/) {
			...
			$which_var = 3;
		} elsif ($_ =~ /^(DIHEDRAL|IMPROPER)$/) {
			$which_var = 4;
		}
		if ($_ =~ /^\s*(\S+)\s+([a-zA-Z0-9\*]+)\s*([^\.]*)$/) { #equivalence?
			if(exists($PARMS{ATOMTYPES}{$1}) and exists($PARMS{ATOMTYPES}{$2})) {
				if($3) {
					@aTypes = split /\s+/,"$2 $3";
				} else {
					@aTypes=($2);
				...
				$validEquiv = 1;
				for $i (@aTypes) {
					if(!exists($PARMS{ATOMTYPES}{$i}) or (substr($1,0,1) ne substr($i,0,1))) { # substr takes care of the case where the hydropholic fftypes are listed
						$validEquiv = 0;
						last;
					}
				}
		...
		} elsif($which_var == 1 && $_ =~ /^\s*(\S+)\s+(\d+\.\d*)/) { #atom types
			$type_counter++;
			if ($2 == 0) {
				$eleNum = 0;
			} else {
				$eleNum = &General::FindElementByMass($2, $elements);
			}
			$rec = (
						{
							"TYPEID"        => $type_counter,
			...
			$rec->{OTHER} = $vals[1] if(defined($vals[1]));
			if (! exists($PARMS{ATOMTYPES}{$1})) {
				$PARMS{ATOMTYPES}{$1} = $rec;
				$EQUIV->{$1} = ();
			}
			$validFF = 1;
		} elsif ($which_var == 2 && $_ =~ /^\s*(\S+)\s*\-(\S+)\s+(\d+\.\d*)\s+(\d+\.\d*)/) { #bonds
			next if (! atomTypeTester($PARMS{ATOMTYPES}, $1, $2));
			$curr = \%{ $PARMS{BONDS} };
			$type_counter=0;
			for $i ($2,$1) {
				last if(! exists($curr->{$i}));
				$curr=\%{ $curr->{$i} };
				$type_counter++;
			}
			$curr = \%{ $PARMS{BONDS}{$1}{$2} } if ($type_counter < 2);
		...
		} elsif ($which_var == 3 && $_ =~ /^\s*(\S+)\s*\-(\S+)\s*\-(\S+)\s+(\d+\.\d*)\s+(\d+\.\d*)/) { #angles
			next if (! atomTypeTester($PARMS{ATOMTYPES}, $1, $2, $3));
			$curr = \%{ $PARMS{ANGLES} };
			$type_counter=0;
			for $i ($3,$2,$1) {
				last if(! exists($curr->{$i}));
			...
			$curr = \%{ $PARMS{ANGLES}{$1}{$2}{$3} } if ($type_counter < 3);
		...
		} elsif ($which_var == 4 && $_ =~ /^\s*(\S+)\s*\-(\S+)\s*\-(\S+)\s*\-(\S+)\s+(\d+)\s+(\d+\.\d*)\s+(\d+\.\d*)\s+(\-?\d+)/) { #torsions
			next if (! atomTypeTester($PARMS{ATOMTYPES}, $1, $2, $3, $4));
			$curr = \%{ $PARMS{TORSIONS} };
			$type_counter=0;
			for $i ($4,$3,$2,$1) {
				last if(! exists($curr->{$i}));
			...
			$curr = \%{ $PARMS{TORSIONS}{$1}{$2}{$3}{$4} } if ($type_counter < 4);
			$validFF = 1;
			$type_counter = findDuplicate($curr,"FOURIER");
			@vals = ($6/$5, abs($8), $7);
			if (! defined($type_counter) or ! $type_counter) {
				$type_counter = 1;
				$curr->{$type_counter} = (
										{
											"INDEX"    => $type_counter,
			...
			} else { #multiple torsions 
				$i = (scalar(@{ $curr->{$type_counter}{VALS} })-1)/3 + 1;
				shift @{ $curr->{$type_counter}{VALS} };
				unshift @{ $curr->{$type_counter}{VALS} }, $i;
				#push @{ $curr->{$type_counter}{VALS} }, $i;
				push @{ $curr->{$type_counter}{VALS} }, @vals;
				#$curr->{$type_counter}{NUM}++;
				$curr->{$type_counter}{BLANK} = "$1 $2 $3 $4";
			}
		} elsif ($which_var == 4 && $_ =~ /^\s*(\S+)\s*\-(\S+)\s*\-(\S+)\s*\-(\S+)\s+(\d+\.\d*)\s+(\d+\.\d*)\s+(\d+)/) { #inversions
			next if (! atomTypeTester($PARMS{ATOMTYPES}, $1, $2, $3, $4));
			$curr = \%{ $PARMS{INVERSIONS}{$3}{$2}{$1}{$4} };
			@vals = ($5, -1, $7) if ($6 == 180);
			@vals = ($5, 1, $7) if ($6 != 180);
			$validFF = 1;
			$type_counter = findDuplicate($curr,"SHFT_DIHDR");
			$type_counter = 1 if (! defined($type_counter) or ! $type_counter);	
			$curr->{$type_counter} = (
										{
											"INDEX"   => $type_counter,
		...
		} elsif ($which_var == 5 && $_ =~ /^\s*(\S+)\s+(\d+\.\d*)\s+(\d+\.\d*)/) { #diagonal vdw
			next if (! atomTypeTester($PARMS{ATOMTYPES}, $1));
			@vals = ($3, $2 * 2/2**(1/6));
			@aTypes = keys %{ $EQUIV->{$1} };
			push @aTypes, $1;
			$validFF = 1;
			for $i (@aTypes) {
				$curr = \%{ $PARMS{VDW}{$i}{$i} };
				$type_counter = findDuplicate($curr,"LJ_6_12");
				$type_counter = 1 if (! defined($type_counter) or ! $type_counter);	
				$curr->{$type_counter} = (
											{
												"TYPE"		=> "LJ_6_12",
	...
	close FORCEFIELD;
	die "ERROR: Invalid AMBER force field file $ff_file->{FF}\n"
		if (! $validFF);
	return \%PARMS;		
}
...
sub parseAmberFF_old {
	...
	while (<INFILE>) {
		...
		if ($_ =~ /^(\w+\*?\-?\+?)\s+\w+\*?\-?\+?/ && $_ !~ /\d+\./ && $section > 4) { #same vdw
			...
			#}
		} elsif ($_ =~ /^\s*(\S+)\s+(\d+\.\d+)\s+(\d+\.\d+)/) { #diagonal vdw
		}
	}
	close INFILE;
...
sub CreateAmberTrj {
	...
	for $dim ("ALPHA", "BETA", "GAMMA") {
		$BOX->{$dim} = 90;
		$BOX->{$dim} = $BBOX->{$dim} if (exists($BBOX->{$dim}));
	}
