...
use Cwd;
use General qw(CoM GetBondLength GetMinDist LoadElements FindElement FindElementByMass);
...
sub BuildAtomSelectionString {
	...
	#special case if the within keyword is used. syntax is within(distance of atom_index) or within(distance of x,y,z)
	while ($sel =~ /within\s*\((\d+\.?\d*)\s+of\s+(\d+\.?\d*),(\d+\.?\d*),(\d+\.?\d*)\)/i) {
		$dist = $1;
		$x = $2; $y = $3; $z= $4;
		$sel =~ s/within\s*\((\d+\.?\d*)\s+of\s+(\d+\.?\d*),(\d+\.?\d*),(\d+\.?\d*)\)/GetBondLength\(\$atoms->\{\$i\}\,\"$x,$y,$z\"\, \$box\) <= $dist/i;
	}
	while ($sel =~ /within\s*\((\d+\.?\d*)\s+of\s+(\d+)\)/i) {
		$dist = $1;
		$atm_no = $2;
		$sel =~ s/within\s*\((\d+\.?\d*)\s+of\s+(\d+)\)/GetBondLength\(\$atoms->\{\$i\}\,\$atoms->\{$atm_no\},\$box\) <= $dist/i;
	}
	return $sel;
}
