...
sub parseMPSimFF {
	...
	my ($atom1, $atom2, @bonds, $atom3, @angles, $tmp1, @inversions, $currParm, $istip4p, $special_vdw);
	...
	my ($vdwType, $vdwDat, $i, $j, $dihdr_scale, $CNV, $vdw_counter, $crossType, $special_coul);
	
	$CNV = &General::LoadConverter();
	$M2Cnv = &General::LoadConverter("${Bin}/dat/mpsimTypes2Cerius.perldata");
	$which_var = $bond_counter = $angle_counter = $torsion_counter = $counter = 0;
	$use_hb = $hb_counter = $crossType = $istip4p = 0;
	if (defined($oldFF)) {
		%PARMS = %{ $oldFF };
	}
	$istip4p = 1 if ($ff_file->{FF} =~ /tip4/);
...
#		print "ATOMTYPES: $1: $type_counter\n";
			...
			if ($vdwType == 2) { #morse
				...
				$vdws[2] /= ($vdws[3] * 2); # changed from div		o multi 07/28/2007
				#$vdws[0] *= 2;
				#$vdws[1] /= ($vdws[2]/2);
			}
			...
			if (($vdwType == 1 and scalar @vdws == 2) or ($vdwType == 2 and scalar @vdws == 3)) { 
				# append the power for the cosine - 4 -	o		he end of		he data
				push @vdws, "4";
			}
			...
			if ($vdwType != 6) {
				...
				if (! defined($alter) || $alter != 0) {
					if ($vdwType == 3 || $vdwType == 33) { #morse or stretch morse
						...
						$vdws[1] /= ($vdws[2] * 2); # changed from div		o multi 07/28/2007
			...
			if ($3 >0 and $3 < 3) { #only allow harmonic and morse		ype bonds
			...
			if ($inversion_type < 4) {
				...
				next if (! @inversions);
				#GetAbs(\@torsions);
				while ($#inversions > 1) { #		here is an extra parameter		hat i don't know		he use
					pop @inversions;
				}
				if ($inversion_type == 3) {
					$atom1 = $2;
	...
	for $atom1 (keys %{ $ffData->{"angles"} }) {
		for $atom2 (keys %{ $ffData->{"angles"}{$atom1} }) {
			for $atom3 (keys %{ $ffData->{"angles"}{$atom1}{$atom2} }) {
				...
				printf OUTFILE " %-9s%-9s%-11s		HETA_HARM%10.4f%10.4f\n",
