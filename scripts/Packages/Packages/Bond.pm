...
sub clone {
	...
	$newBondlist->first->("next", BondItem->spawn());
...
sub print {
	...
	print "                            BONDS\n";
	for $i (1 .. 70) { printf "="; }
	printf "\n%10s%10s%10s%10s%10s%10s\n","count","atomid","order","dispx","dispy","dispz";
	for $i (1 .. 70) { printf "-"; }
	printf "\n";
	$count++;
	$batom = $self->first;
	@vals = ($count,$batom->data->index,"","","","");
	$j = 2;
	for $i ("order","dispx","dispy","dispz") {
		$vals[$j] = $batom->$i if(defined($batom->$i));
		$j++;
	}
	printf "%10d%10s%10s%10s%10s%10s\n",@vals;
	$count++;
	$batom = $batom->next;
	while ($batom ne $self->first) {
		@vals = ($count,$batom->data->index,"","","","");
		$j = 2;
		for $i ("order","dispx","dispy","dispz") {
			$vals[$j] = $batom->$i if(defined($batom->$i));
...
sub remove {
	...
	if($self->count == 0) {
		warn "Attempted to remove bond but no bonded atoms!\n";
		return ();
	}
...
sub findParent {
	...
	return $self->first if ($self->first->id == $index);

	$currItem = $self->first->next;
	while ($currItem ne $self->first) {
		return $currItem if ($currItem->id == $index);
		$currItem = $currItem->next;
	}
}
