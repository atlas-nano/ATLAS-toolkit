...
use FindBin qw($Bin);
use lib "$FindBin::Bin";
use ManipAtoms qw(GetMols CreateBondsByDistance);
use AMBER qw(getOpts getTopInfo parseAmberCoordFile getConn GetVels);
use CHARMM qw(parsePSFFile parseCharmmCoordFile);
use GROMACS qw(parseGroTopFile parseGroFile ParseGroFF);
use BOX qw(MakeBox);
use General qw(Trim FileTester);
...
@ISA = qw(Exporter);
@EXPORT = ();
@EXPORT_OK = qw(createBGF GetBGFFileInfo addElement addCon addHeader sortByRes createMOL2 createMSI GetMSIFileInfo
				...
				insertHeaderRemark InsertMol H2others GetXYZFileInfo GetMOL2Header WriteSWfile GetCHARMMFileInfo GetAMBERFileInfo
				ParseStructFile GetCIFFileInfo);
$VERSION = "1.00";
...
sub createBGF {
	...
	for $Atm_no (sort numerically keys %{ $AtmData }) {
		...
		$atom->{"NUMBONDS"} = 9 if ($atom->{"NUMBONDS"} > 9);
		$atom->{"LONEPAIRS"} = 0 if (! exists($atom->{"LONEPAIRS"}));
		$atom->{"LONEPAIRS"} = 0 if ($atom->{"LONEPAIRS"}>9);
		...
		#if($atom->{FFTYPE} =~ /_shell$/ and length($atom->{FFTYPE}) > 5) {
		#	$atom->{FFTYPE} = substr($atom->{FFTYPE},0,5);
		#}
		printf OUTDATA "%-6s %5d %-5s %3s %1s %5d%10.5f%10.5f%10.5f %-5s %2d %1d %9.6f",
...
sub GetBGFFileInfo {
	my ($in_file, $saveheaders, $verbose) = @_;
	...
	$atm_patern = '^ATOM\s*(\d+)\s+(\S+)\S?\s+(\S+)\s\w?\s*(\d+)\s*' . 
...
sub GetPDBFileInfo {
	my ($infile, $saveHeaders, $verbose, @FFs) = @_;
	my ($atom_no, $inText, $FData, $rec, $isvalid, $BONDS); 
	my ($fftype, $i, $tmp, $res_nm, $FFILES, $aCharge, $aRadii, $PARMS);

	$isvalid = 0;
	open INFILE, $infile or die "Cannot open $infile: $!\n";
	while (<INFILE>) {
		chomp;
		undef $aCharge; undef $aRadii;
		$inText = $_;
		if ($inText =~ /^(ATOM  |HETATM)(.{5})(.{6})\s*(\S+)\s+\w*\s+(\d+)\s+(\-?\d+\.\d+)\s*(\-?\d+\.\d+)\s*(\-?\d+\.\d+)\s*(\-?\d*\.*\d*)\s*(\d*\.*\d*)/g) {
			$res_nm = $4;
			$atom_no = $2;
			$fftype = $3;
			$rec = (
			...
			$aCharge = $9 if (defined($9));
			$aRadii = $10 if (defined($10));
			$atom_no =~ s/\s+//g;
			$rec->{FFTYPE} = $fftype;
			$rec->{FFTYPE} =~ s/^\s+//;
			$rec->{FFTYPE} =~ s/\s+$//;
...
#			if (defined($hasRadii) and $hasRadii eq "1") {
			...
			$FData->{$atom_no}{CHARGE} = $aCharge if(defined($aCharge));
			$FData->{$atom_no}{RADII} = $aCharge if(defined($aRadii));
			 ...
			 $atom_no =~ s/\s+$//;
			 while ($rec =~ /(.{5})/g) {
				push @{ $BONDS->{$atom_no} }, $1+0;
				push @{ $BONDS->{$1+0} }, $atom_no;
			 }
		}
	}
	close INFILE;
	...
	if (@FFs) {
		#update fftype info from ITP files if provided
		($FFILES, undef) = General::ReadFFs("@FFs", undef);
		$PARMS = &General::LoadFFs($FFILES, undef, 0, $FData, \%{ $BONDS });
		#now update the charge from the forcefield
		for $i (values %{ $FData }) {
			$fftype = $i->{FFTYPE};
			if(exists($PARMS->{ATOMTYPES}{$fftype}) and
			   exists($PARMS->{ATOMTYPES}{$fftype}{CHARGE_SET}) and
			   $PARMS->{ATOMTYPES}{$fftype}{CHARGE_SET}) {
			   $i->{CHARGE} = $PARMS->{ATOMTYPES}{$fftype}{CHARGE};
		   }
	   }
	}
	if (! keys %{ $BONDS} ) {
		#Guess bonds if not defined
		for $i (keys %{ $FData }) {
			$tmp->{$i} = 1;
		}
		&ManipAtoms::CreateBondsByDistance(\%{ $FData }, \%{ $BONDS }, undef, $tmp, undef, 4);
	}
	return (\%{ $FData }, \%{ $BONDS });
}

sub GetXYZFileInfo {
	my ($infile, $calcBonds) = @_;
	my ($i, $inText, $natom, $dostart, $moln, $icount, $FData, $rec, @HEADER, $BData, $tmp);
	...
	&ManipAtoms::CreateBondsByDistance(\%{ $FData }, \%{ $BData }, undef, $tmp) if($calcBonds);
	return (\%{ $FData } ,\%{ $BData },\@HEADER);
}

sub GetMOL2FileInfo {
	my ($in_file, $calcBox, $verbose) = @_;
	my ($isAtm, $isBond, $atm_patern, $bond_patern, $isHeader); 
	my (%ATOMS, $description, $order, %CONN, $tmp, $HEADERS);
	...
	$molname =~ s/\.\w+$//;
	push @{ $HEADERS }, "BIOGRF 200";
	push @{ $HEADERS }, "DESCRP $molname";
	push @{ $HEADERS }, "REMARK $description";
	push @{ $HEADERS }, "REMARK BGF file created by mol2bgf";
	push @{ $HEADERS }, "FORCEFIELD DREIDING";
	return (\%ATOMS, \%CONN, $HEADERS);
}
...
sub createMOL2 {
	...
	my ($atom, $bond, @atmList, $bList, $aList, $sList, $cRes, $resID); 
	my ($resName, $RESLIST, $i, $j, $c, $order, $title);
	...
	for $atom (@atmList) {
		...
		if (! defined($cRes) or $cRes != $resID) {
			$c++;
			$cRes = $resID;
			$sList .= sprintf("%7d%5s%15d ****               0 ****  ****\n",$c,$resName,$atom);
		}
		$aList .= sprintf("%7d %-4s %13.4f%10.4f%10.4f %-4s%7d%4s%16.6f\n",
	...
	print MOL2 "$j $i $c 0 0\n";
	print MOL2 "SMALL\nUSER_CHARGES\n\n\@<TRIPOS>ATOM\n";
	print MOL2 "${aList}@<TRIPOS>BOND\n";
	print MOL2 "${bList}@<TRIPOS>SUBSTRUCTURE\n${sList}";
	close MOL2;	
}
...
sub createPDB {
	...
	my ($fmt) = '%-6s%5d%4s  %3s %1s%4d %11.3f%8.3f%8.3f';
...
sub AMBER2MOL2Types {
	...
	$convertFile = "$Bin/dat/amber2mol2Types.perldata";
...
sub createMSI {
    ...
    my ($out_string, $counter, $index, $h_keys, $h_vals, $data_type);
    ...
    for $index (sort Numerically keys %{ $ATOMS }) {
		...
		for $h_keys ("ACL", "Charge", "FFType", "XYZ", "Id", "Label") {
	    	if ($h_keys eq "XYZ") {
				...
				$data_type = "C";
				if ($h_keys ne "Label") {
					$h_vals = $ATOMS->{$index}{uc($h_keys)};
				} else {
					$h_vals = $ATOMS->{$index}{ATMNAME};
				}
				$out_string .= "  (A $data_type $h_keys " . '"' . $h_vals . '")' . "\n";
	    	}   
		}
		$out_string .= " )\n";
		$counter++;
...
sub GetCHARMMFileInfo {
	my ($psfFile, $saveheaders, $verbose, $coordFile) = @_;
	my ($atoms, $bonds, $headers);

	$verbose = 0 if (! defined($verbose));
	($coordFile) = charmmFileCheck($psfFile, $verbose) if (! defined($coordFile));
	print "Obtaining Topology Information from ${psfFile}..." if ($verbose);
	($atoms, $bonds) = parsePSFFile($psfFile);
	print "Obtaining Coordinate Information from ${coordFile}..." if ($verbose);
	&parseCharmmCoordFile($atoms, $coordFile);
	if($saveheaders) {
		$headers = createHeaders(undef, $psfFile);
		return ($atoms, $bonds, $headers);
	} else {
		return ($atoms, $bonds);
	}
...
sub charmmFileCheck {
	my ($psfFile, $verbose) = @_;
	my ($coordFile);

	$coordFile = $psfFile;
	$coordFile =~ s/\.\w+/\.crd/;
	print "Searching for CHARMM coordinate file $coordFile..." if ($verbose);
	&General::FileTester($coordFile);

	return $coordFile;
}

sub GetAMBERFileInfo {
	my ($topFile, $saveheaders, $verbose, $coordFile, $cnv_file) = ($_[0], $_[1], $_[2]);
	my ($OPTS, $DATA, $RES, $timeStamp, $hasVels, $CONS, $HEADERS, $vels, $totAtms); 

	$verbose = 0 if (! defined($verbose));
	($coordFile, $cnv_file) = amberFileCheck($topFile, $coordFile, $cnv_file, $verbose);
	print "Obtaining Topology/Coordinate Information..." if ($verbose);
	$OPTS = &AMBER::getOpts();
	...
	print "Generating Connectivities..." if ($verbose);
	$CONS = getConn(\%{ $DATA->{"BONDLIST"} }, $DATA->{"ATOMS"});
	if (keys %{ $DATA->{BOX} }) {
		$DATA->{BOX} = BOX::MakeBox($DATA->{BOX});
		$HEADERS = createHeaders(\%{ $DATA->{"BOX"} }, $topFile);
	} else {
		$HEADERS = createHeaders(undef, $topFile);
	}

	if($saveheaders) {
		return (\%{ $DATA->{ATOMS} }, $CONS, \@{ $HEADERS });
	} else {
		return (\%{ $DATA->{ATOMS} }, $CONS);
	} 
}

sub amberFileCheck {
	my ($topFile, $coordFile, $cnv_file, $verbose) = @_;

	print "Searching for AMBER coordinate file..." if ($verbose);
	if (! defined($coordFile)) {
		$coordFile = $topFile;
		$coordFile =~ s/\.\w+/\.crd/;
		$coordFile =~ s/\.\w+/\.rst7/ if (! -e $coordFile or ! -r $coordFile or ! -T $coordFile);
	}

	print "Searching for AMBER residue conversion file..." if ($verbose);
	if (defined($cnv_file)) {
		&General::FileTester($cnv_file);
	} else {
		$cnv_file = $topFile;
		$cnv_file =~ s/\.\w+/\.cnv/;
		undef $cnv_file if (! -e $cnv_file or ! -r $cnv_file or ! -T $cnv_file);
	}

	return ($coordFile, $cnv_file)
}

sub GetGromacsFileInfo {
	my ($groFile, $saveheaders, $verbose, $topFile) = @_;
	my ($atoms, $bonds, $box, $headers, $groRef);

	$verbose = 0 if (! defined($verbose));
	($topFile) = gromacsFileCheck($groFile, $verbose) if (! defined($topFile));
	print "Obtaining coordinate information from ${groFile}..." if ($verbose);
	($atoms, undef, $box) = GROMACS::parseGroFile(undef,$groFile, 0);
	print "Obtaining Gromacs/XPLOR Topology information from ${topFile}..." if ($verbose);
	#&GROMACS::parseGroTopFile($topFile,$atoms, \%{ $bonds }, 1);
	$groRef->{atoms} = $atoms; $groRef->{box} = $box;
	($atoms, $bonds) = GROMACS::ParseGroFF($topFile, 2, 0, $groRef);
	if($saveheaders) {
		$headers = createHeaders($box, $topFile);
		return ($atoms, $bonds, $headers);
	} else {
		return ($atoms, $bonds);
	}
...
sub gromacsFileCheck {
	my ($groFile, $verbose) = @_;
	my ($topFile);

	$topFile = $groFile;
	$topFile =~ s/\.\w+$/\.top/;
	print "Searching for GRO file $groFile..." if ($verbose);
	if (! -e $topFile or ! -T $topFile) {
		$topFile = $groFile;
		$topFile =~ s/\.\w+$/\.xplor/;
	}
	&General::FileTester($topFile);

	return $topFile;
}

sub ParseStructFile {
	my ($inStr, $saveHeaders, $verbose, $fType) = @_;
	my ($inFile, $inputFileType, $readFile, $atoms, $bonds, $headers);

	@{ $inFile } = split /\s+/,$inStr;
	for (@{ $inFile }) {
		&General::FileTester($_);
	}
	if (defined($fType)) {
		$inputFileType = $fType;
	} else {
		#here we get the file suffix and set the readFile variable appropriately
		$inputFileType = $inFile->[0];
		$inputFileType =~ /^.*\.(\w+)$/;
		$inputFileType = uc $1;
	}
	if ($inputFileType =~ /BGF/i) {
		$readFile = \&GetBGFFileInfo;
	} elsif ($inputFileType =~ /PDB|PQR/i) {
		$readFile = \&GetPDBFileInfo
	} elsif ($inputFileType =~ /MOL2/i) {
		$readFile = \&GetMOL2FileInfo;
	} elsif ($inputFileType =~ /XYZ/i) {
		$readFile = \&GetXYZFileInfo;
	} elsif ($inputFileType =~ /PSF/i) {
		$readFile = \&GetCHARMMFileInfo;
	} elsif ($inputFileType =~ /PRMTOP/i) {
		$readFile = \&GetAMBERFileInfo;
	} elsif ($inputFileType =~ /GRO/i) {
		$readFile = \&GetGromacsFileInfo;
	} elsif ($inputFileType =~ /MSI/i) {
		$readFile = \&GetMSIFileInfo;
	} elsif ($inputFileType =~ /CIF/i) {
		$readFile = \&GetCIFFileInfo;
	} else {
		die "ERROR: Unknown file type $inputFileType detected!\n"
	}
	if($#{$inFile} == 0) {
		($atoms, $bonds, $headers) = $readFile->($inFile->[0], $saveHeaders, $verbose);
	} else {
		($atoms, $bonds, $headers) = $readFile->(shift @{ $inFile }, $saveHeaders, $verbose, @{ $inFile });
	}
	return ($atoms,$bonds,$headers);
}
...
#sub GetCIFFileInfo {
#	my ($infile) = $_[0];
#	my ($inText, @headers, $crystx, $aID);
#	my (%ATOMS, %BONDS, $tmp);
#
#	$crystx = "CRYSTX ";
#	$aID = 0;
#	#first apply symmetry operations.... use atomsk for this
#	system("rm -fr __test.cif; atomsk $infile __test.cif;");
#	$infile = "__test.cif";
#	#now open the P1 cif file and read cell and atom data
#	open INFILE, $infile or die "Cannot open $infile: $!\n";
#	while (<INFILE>) {
#		chomp;
#		$inText = $_;
#		if ($inText =~ /^(_chemical|_audit)/) {
#			push @headers, "REMARK: $inText";
#		} elsif ($inText =~ /^(_cell_length|_cell_angle/) {
#			$crystx .= "$2 ";
#		} elsif ($inText =~ /^loop/) {
#			push @headers, $crystx;
#			$aID = 1;
#		} elsif ($inText =~ /^(\S+)\s+(\w+)\s+(\-?\d+\.\d+)\s+(\-?\d+\.\d+)\s+(\-?\d+\.\d+)/ and $atom_start == 1) {
#			$ATOMS{$aID} = (
#								{
#									"INDEX"		=> $aID++,
#									"ATMNAME"	=> $1,
...
#		$tmp->{$i} = 1;
#	}
#	&CreateBondsByDistance(\%ATOMS, \%BONDS, undef, $tmp);
#	return (\%ATOMS ,\%BONDS,\@headers);
#}
sub Numerically { ($a<=>$b) }
1;
