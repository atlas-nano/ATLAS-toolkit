...
sub parseCharmmFF {
	my ($ff, $alter, $parms) = @_;
	my ($i, $j, $sstr, $header, $ty_counter, $ele, $CNV, $rec); 
	my ($curr, $tmp, $t1, $t2, $inv, $cmap, $valid, $ty_index);
		...
		$parms->{PARMS} = (
							{
								...
								'scale_cou_12'							=> 0,
								'scale_cou_13'							=> 0,
								'scale_cou_14'							=> 0.00000,
								'scale_torsions'						=> 0,
								'scale_torsions_by_n_defined_torsions'	=> 0,
								'scale_vdw'								=> 0.00000,
								'scale_vdw_12'							=> 0,
								'scale_vdw_13'							=> 0,
								'scale_vdw_14'							=> 0.00000,
								'single_inversion'						=> 1,
							}
		);
	}
    ...
    while (<CHARMMPAR>) {
		...
		if ($sstr =~ /^ATOMS/) {
			$header = "ATOMTYPES";
		} elsif ($sstr =~ /^(BONDS|ANGLES|DIHEDRALS|IMPROPER|NONBONDED|CMAP)/) {
			undef $ty_counter; 
			undef $curr;
			$ty_index = 0;
			$header = $1;
		...
		} elsif ($header eq "ATOMTYPES" and $_ =~ /^MASS\s+\-?(\d+)\s+(\w+)\s+(\d+\.\d+)\s+/) { #atoms
			$parms->{$header}{$2} = (
										{
											"TYPEID"     => $1,
											"LABEL"      => $2,
