...
sub GetPDBFileInfo {
	...
	my ($inText, $FData, $rec, $isvalid, $BONDS, $fftype, $i, $tmp);
	$isvalid = 0;
	...
	&CreateBondsByDistance(\%{ $FData }, \%{ $BONDS }, undef, $tmp, undef, 4) if(! keys %{ $BONDS });
	return (\%{ $FData }, \%{ $BONDS });
}
...
sub GetXYZFileInfo {
	...
	&CreateBondsByDistance(\%{ $FData }, \%{ $BData }, undef, $tmp) if($calcBonds);
