...
@EXPORT_OK = qw(IsInteger Swap CenterText STDev IsDecimal IsBoolean Trim ParseSanderInfoFile TrjSelections
				...
				GetFileTypeStr GetFFTypeStr FindElementByMass LoadConverter);
$VERSION = "1.00";
	...
	for $i (@{ $ffs }) {
		...
		if (ref($i) ne "HASH") {
			...
			if ($i->{FFTYPE} =~ /CERIUS/i) {
				...
				$PARMS = &parseOplsItpFF($i,$alter, $PARMS);
...
sub LoadElements {
	...
	while (<INDATA>) {
		...
		if ($indata =~ /^(\d+\.?\d*)\s+(\*?)\s*\d+/) {
			...
			#$indata = $'; 
			if ($indata =~ /^\d+\.?\d*\s+\*?\s*(\d+\.?\d*)\s+(\w+)\s+(\w+)\s+(.+)$/) {
				$ELEMENTS{$eleNum} = (
										{
											"MASS"      => $1,
											"NAME"      => $2,
...
sub FindElementByMass {
   ...
   for $i (keys %{ $elements }) {
    $eMass->[0] = $elements->{$i}{MASS}-.5;
    $eMass->[1] = $elements->{$i}{MASS}+.5;
