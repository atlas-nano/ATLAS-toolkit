...
use General qw(FileTester GetSelections LoadFFs TrjSelections);
use LAMMPS qw(ParseLAMMPSTrj GetLammpsByteOffset GetLammpsTrjType ConvertLammpsBox);
use FileFormats qw(GetBGFFileInfo addHeader AddMass createHeaders createBGF);
use File::Basename qw(basename);
use Getopt::Std qw(getopt);
use ManipAtoms qw(GetAtmList UnwrapAtoms);
use Superimpose;
use Math::MatrixReal;
...
sub alignMols {
	...
	@tmp = grep /\d+/, keys %{ $fData->{$i} };
		...
		print "Using timestep $tStep as reference\n" if ($doPrint);
                ...
                &Kabash::SuperimposeAtoms($refBGF, $matchBGF);
            }
	    $rec = (
		    {
			"DATA"  => $matchBGF,
